...
- serverside request forgery occurs when app is fetching a remote resource without validating the user supplier URL


## PortSwigget Academy:
## Insecure direct object references (IDOR)
- access control vulnerability that appea when an application uses user-supplied input to access objects directly
- for example by modifying customer number at the end of an url like: URL https://insecure-website.com/customer_account?customer_number=132355 , would be able to see other customers info
...
- Server-side template injection attacks can occur when user input is put directly into a template, rather than passed in as data
- attacker is able to use native template syntax to inject a malicious payload into a template, which is executed server-side.
...
- tätä voi testata esim alert() -komennolla
...
## h) Reflected XSS into HTML context with nothing encoded
...
## i) Stored XSS into HTML context with nothing encoded
...
Tässä kohtaa tämäkin virtuaalikone alkaa hidastelemaan ja jumittamaan niin pahasti, ettei käytöstä tule enää mitään. En nyt uskalla enää sulkea mitään ja käynnistellä uusiksi, ettei jotain hajoa, joten pitää  nyt vaan toivoa että nopeutuisi kohta.
